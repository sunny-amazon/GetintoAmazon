...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'getintoamazonweb'),
        'collapse' => __('collapse child menu', 'getintoamazonweb'),
    ));
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'getintoamazonweb'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'getintoamazonweb'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'getintoamazonweb');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'getintoamazonweb'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'getintoamazonweb'));
}
...
function getProductButtonHtml ($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '') {
    ...
    $buttonText = sprintf(__('Add to cart', 'getintoamazonweb'));
    if ($contentProductbutton !== '') {
        $buttonText =  sprintf(__($contentProductbutton, 'getintoamazonweb'));
    }
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'getintoamazonweb'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'getintoamazonweb')
    ));
    extract($args);
